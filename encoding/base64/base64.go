...
// the standard encoding with - and _ substituted for + and /(for url).
type Encoding struct {
	encode    [64]byte
	decodeMap [256]byte // use array and not map to save space
	padChar   rune
	strict    bool
}
...
func NewEncoding(encoder string) *Encoding {
	...
	copy(e.encode[:], encoder) // have to copy, since encode is array
...
// 不进行append处理，dst完全使用index访问，当dst长度不足时可能panic: index out of range
// 这样速度更快，缺点是需要提前计算dst的长度并分配
func (enc *Encoding) Encode(dst, src []byte) {
	if len(src) == 0 {
		return
	}

	di, si := 0, 0 // destinition pointer, source pointer
	n := (len(src) / 3) * 3
	for si < n {
	...
	remain := len(src) - si // 多出的字节, 1 or 2
	if remain == 0 {
		return
	}
...
// encoder is streaming base64 encoder
type encoder struct {
	err  error
	enc  *Encoding
	w    io.Writer
...
// 结束写后Close是必须操作，否则buf可能有残留数据未flush
...
// 保证解码中间出错不影响dst, 只有src中的4个byte对应的字符均不是0xff时才写dst
...
func (r *newlineFilteringReader) Read(p []byte) (int, error) {
	...
	for n > 0 {
		offset := 0
		for i, b := range p[:n] { // left shift, watch closely
			if b != '\r' && b != '\n' {
				if i != offset {
					p[offset] = b
				}
		...
		// Previous buffer entirely LR or CR, read again
		n, err = r.wrapped.Read(p)
	}
	return n, err
}
